...
def main(args):
    ...
    prenotati = (
        ...
        .sort_values(by="COGNOME")
        .set_index("MATRICOLA")
    )
    prenotati = prenotati.assign(AULA=np.NaN, POSTO=np.NaN)

    # prenotati.NOTE = prenotati.NOTE.astype('str')
    if not prenotati.NOTE.dtype == "object":
        prenotati.NOTE = ""
    matricole = prenotati[
        ~prenotati.NOTE.str.contains("Esame online", na=False)
    ].index.to_list()

    if args.dsa_room is not None:
        matricole_dsa = prenotati[
            prenotati.NOTE.str.contains("Dsa", na=False)
            | prenotati.NOTE.str.contains("Tempo aggiuntivo", na=False)
        ].index.to_list()
        print(
    ...
    for i, (a, room_name) in enumerate(zip(args.rooms, room_names)):
        ...
        if "filename" not in config and args.folder:
            ...
            if not config["filename"].exists():
                config["filename"] = (
                    Path.cwd() / "Aule esami Polito, disposizione posti.xlsx"
                )
        ...
        prenotati[prenotati.AULA == room_name].to_excel(
            writer_p, sheet_name=f"Aula_{room_name}"
        )
    ...
    if len(matricole) != 0:
        raise ValueError(
            f"⚠️  Designated rooms are not sufficient, {len(matricole)} students missing ({matricole})"
        )

    if args.nopc_students:
        prenotati[prenotati.AULA == args.nopc_room].to_excel(
            writer_p, sheet_name=f"Aula_{args.nopc_room}"
        )
    ...
    with open(f"{args.name}_limiti.txt", "w") as f:
        limiti = (
            prenotati[prenotati.AULA != args.nopc_room]
            .groupby("AULA")
            .agg({"COGNOME": ["min", "max"]})
        )
        f.write(str(limiti))
